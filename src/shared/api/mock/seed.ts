...
const LEAD_SOURCES = ['Instagram', 'Telegram', 'Friend referral', 'Website', 'Walk-in', 'Facebook'] as const;
const LEAD_STATUSES = ['new', 'contacted', 'trial', 'negotiation', 'won', 'lost'] as const;

const PAYMENT_METHODS = ['cash', 'card', 'click', 'payme', 'transfer'] as const;
const STUDENT_STATUSES = ['active', 'trial', 'frozen', 'archived'] as const;
const TEACHER_STATUSES = ['active', 'on_leave', 'inactive'] as const;
...
const phone = () =>
  `+998 ${[90, 91, 93, 94, 97, 99, 88, 33][between(0, 7)]} ${between(100, 999)} ${between(10, 99)} ${between(10, 99)}`;

const email = (first: string, last: string) =>
  `${first.toLowerCase().replace(/[^a-z]/g, '')}.${last.toLowerCase().replace(/[^a-z]/g, '')}@gmail.com`;
...
export interface SeedTeacher {
  ...
  specialty: string;
  branch: string;
  status: (typeof TEACHER_STATUSES)[number];
...
export interface SeedAttendance {
  date: string;
  groupId: string;
  records: { studentId: string; status: 'present' | 'absent' | 'late' | 'excused'; reason?: string }[];
}
...
/* ------ Build teachers ------ */
export const TEACHERS: SeedTeacher[] = Array.from({ length: 14 }).map((_, i) => {
...
/* ------ Build groups ------ */
export const GROUPS: SeedGroup[] = Array.from({ length: 22 }).map((_, i) => {
  const course = pick(COURSES);
  const teacher = pick(TEACHERS);
  const cohort = Math.floor(i / 4) + 1;
  const days =
    rand() > 0.5 ? ['Mon', 'Wed', 'Fri'] : rand() > 0.5 ? ['Tue', 'Thu', 'Sat'] : ['Mon', 'Tue', 'Wed', 'Thu'];
...
/* ------ Build students ------ */
export const STUDENTS: SeedStudent[] = Array.from({ length: 240 }).map((_, i) => {
  ...
  const tags: string[] = [];
  if (rand() > 0.85) tags.push('VIP');
  if (rand() > 0.9) tags.push('Olympiad');
  if (rand() > 0.93) tags.push('Scholarship');
  if (rand() > 0.7) tags.push(pick(STUDENT_TAGS));

  const status =
    rand() > 0.92 ? 'frozen' : rand() > 0.85 ? 'trial' : rand() > 0.97 ? 'archived' : 'active';
...
/* ------ Build payments ------ */
export const PAYMENTS: SeedPayment[] = [];
...
for (const student of STUDENTS) {
  ...
  for (let m = months; m >= 0; m--) {
    ...
    const rNum = rand();
    const status =
      rNum > 0.92 ? 'overdue' : rNum > 0.86 ? 'pending' : rNum > 0.02 ? 'paid' : 'refunded';
    const paidAt =
      status === 'paid'
...
export const LEADS: SeedLead[] = Array.from({ length: 64 }).map((_, i) => {
  const { firstName, lastName } = buildName();
  const fullName = `${firstName} ${lastName}`;
  return {
    id: id('lead', i + 1),
...
for (const group of GROUPS.slice(0, 12)) {
  for (let d = 0; d < 30; d++) {
    ...
    if (!group.schedule.days.includes(day)) continue;
    ATTENDANCE.push({
      date: date.format('YYYY-MM-DD'),
      groupId: group.id,
      records: group.studentIds.map((sid) => {
        const r = rand();
        const status: SeedAttendance['records'][number]['status'] =
          r > 0.92 ? 'absent' : r > 0.88 ? 'late' : r > 0.86 ? 'excused' : 'present';
...
export const TASKS: SeedTask[] = TASK_TITLES.map((title, i) => ({
  id: id('task', i + 1),
  title,
  ...
  priority: (['low', 'medium', 'high', 'urgent'] as const)[between(0, 3)]!,
  status: (['todo', 'in_progress', 'review', 'done'] as const)[between(0, 3)]!,
...
export const ACTIVITY: SeedActivity[] = Array.from({ length: 40 }).map((_, i) => ({
  id: id('act', i + 1),
  ...
  kind: (['create', 'update', 'delete', 'login', 'export'] as const)[between(0, 4)]!,
}));
